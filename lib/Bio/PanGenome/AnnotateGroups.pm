...
has '_groups_to_consensus_gene_names' => ( is => 'ro', isa => 'HashRef', lazy => 1, builder => '_build__groups_to_consensus_gene_names' );
...
sub _build__ids_to_gene_names
{
  ...
  {
    my $gene_names_from_gff = Bio::PanGenome::GeneNamesFromGFF->new(
      gff_file   =>  $filename
    );
...
sub reannotate
{
  ...
  for my $group_name (sort{ @{$groups_to_id_names{$b}} <=> @{$groups_to_id_names{$a}} }  keys %groups_to_id_names)
  {
    my $consensus_gene_name = $self->_groups_to_consensus_gene_names->{$group_name};
    print {$self->_output_fh} $consensus_gene_name.": ".join("\t",@{$self->_groups_to_id_names->{$group_name}})."\n";
  }
