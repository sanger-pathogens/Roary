...
has '_number_of_genes_per_file' => ( is => 'ro', isa => 'HashRef',   lazy => 1, builder => '_builder__number_of_genes_per_file' );
has '_genes_to_file'            => ( is => 'ro', isa => 'HashRef',   lazy => 1, builder => '_builder__genes_to_file' );
has '_freq_groups_per_genome'   => ( is => 'ro', isa => 'ArrayRef',   lazy => 1, builder => '_builder__freq_groups_per_genome' );
...
sub _builder__freq_groups_per_genome
{
  ...
  open(my $fh, $self->groups_filename) or Bio::PanGenome::Exceptions::FileNotFound->throw(error => "Group file not found:".$self->groups_filename);
  while(<$fh>)
  {
    chomp;
    my $line = $_;
